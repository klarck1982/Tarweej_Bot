...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        for theme in ("light", "dark"):
            ...
            # v0.7.0: ⚙️ المزيد ← بطاقة Nour + منطقة الخطر ← صفحة التصفير
            await page.evaluate("document.getElementById('pc-page').classList.add('hidden')")
            await page.click('.tab[data-tab="more"]'); await page.wait_for_timeout(150)
            await page.screenshot(path=f"{out}/{theme}_p6_more_nour.png")
            if theme == "light":
                await page.click("#nour-test-btn"); await page.wait_for_timeout(500); print("nour test toast:", await page.inner_text("#toast"))
                el = await page.query_selector(".danger"); await el.scroll_into_view_if_needed(); await page.wait_for_timeout(100)
                await page.screenshot(path=f"{out}/light_p7_danger.png")
                await page.click('button[onclick="openReset()"]'); await page.wait_for_timeout(400)
                print("reset btn disabled before word:", await page.evaluate('document.getElementById("reset-btn").disabled'))
                await page.fill("#reset-input", "تصفير"); await page.wait_for_timeout(100)
                print("reset btn disabled after word:", await page.evaluate('document.getElementById("reset-btn").disabled'))
                await page.screenshot(path=f"{out}/light_p8_reset.png", full_page=True)
                await page.evaluate("window.__bbf && window.__bbf()"); print("back closes reset:", await page.evaluate('document.getElementById("reset-page").classList.contains("hidden")'))
